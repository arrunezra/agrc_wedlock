...
try {
    if (!empty($data->refresh_token)) {
        ...
        // 1. JOIN with profiles table to get profile_id and gender
        $sql = "SELECT u.id, u.userid, u.phoneNumber, u.email, u.role, p.profile_id, p.gender, p.is_visible
                FROM users u
                LEFT JOIN profiles p ON u.userid = p.userid
                WHERE u.refresh_token = ? AND isActive = 1
                LIMIT 1";
        ...
        if ($user) {
            // 2. Generate a new Access Token with the FULL payload
            $newPayload = [
                "uid" => $user['userid'],
